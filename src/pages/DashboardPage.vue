...
// Refresh stats whenever tickets update
const refreshStatsListener = () => fetchStats();

onMounted(() => {
  fetchStats();
  window.addEventListener("tickets-updated", refreshStatsListener);
});

onBeforeUnmount(() => {
  window.removeEventListener("tickets-updated", refreshStatsListener);
});
</script>
